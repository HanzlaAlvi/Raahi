...
function InputField({ icon, placeholder, value, onChangeText, keyboardType, maxLength }) {
...
export default function ForgetPasswordScreen({ navigation }) {
  ...
  const isValid = (num) => /^03[0-9]{9}$/.test(num.trim());
  ...
  const handleSendOTP = async () => {
    ...
    if (!isValid(trimmed)) {
      Alert.alert('Invalid Number', 'Please enter an 11-digit Pakistani number starting with 03.');
      return;
    }

    const controller = new AbortController();
    // 65 second timeout — accounts for Render free tier cold start (up to 60s)
    ...
    try {
      ...
      if (response.status === 404) {
        Alert.alert(
          'Number Not Found',
          data.message || 'This number is not registered. Please check and try again.'
        );
        return;
      }

      if (response.status === 400) {
        Alert.alert('Invalid Input', data.message || 'Please check your number and try again.');
        return;
      }
      ...
      console.log('[OTP] Masked number:', data.maskedNumber);
      console.log('[DEV] OTP code:', data.otp);

      // Dev mode: show OTP in alert so you can test without SMS
      Alert.alert(
        'OTP Ready ✅',
      ...
      if (err.name === 'AbortError') {
        Alert.alert(
          ...
          'The server may be waking up (Render free tier takes up to 60 seconds). Please wait a moment and try again.',
          [{ text: 'OK' }]
        );
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={s.topBand}>
          ...
          <View style={s.iconCircle}>
            <Ionicons name="phone-portrait-outline" size={48} color={C.green} />
          </View>
        </View>
        ...
        <View style={s.card}>
          ...
          <Text style={s.desc}>
            Enter your registered Pakistani phone number.{'\n'}We will generate an OTP for you.
          </Text>
          ...
          <View style={s.fields}>
            ...
            {mobileNumber.length > 0 && mobileNumber.length < 11 && (
              <Text style={s.hint}>{11 - mobileNumber.length} more digits needed</Text>
            )}
            {mobileNumber.length === 11 && !isValid(mobileNumber) && (
              <Text style={s.hintError}>Number must start with 03</Text>
            )}
          </View>
          ...
          <TouchableOpacity
            style={[s.btn, (loading || mobileNumber.trim().length !== 11) && { opacity: 0.6 }]}
            onPress={handleSendOTP}
            disabled={loading || mobileNumber.trim().length !== 11}
            activeOpacity={0.85}
          >
            {loading
...
const s = StyleSheet.create({
  ...
  input:     { flex: 1, fontSize: 16, color: C.textMain, fontWeight: '500', letterSpacing: 1 },
  hint:      { color: C.textMuted, fontSize: 12, marginLeft: 4, fontWeight: '500' },
