// frontend/Driver/screens/DriverProfileScreen.js
...
// ── Validation (same as DriverRegisterScreen) ────────────────────────────────
const RULES = {
  name: v => {
    const s = (v||'').trim();
  ...
  license: v => {
    ...
    if (!/^[A-Za-z0-9]{16}$/.test(s)) return 'License number must be exactly 16 alphanumeric characters (letters & numbers only).';
    return null;
  },
  vehicleNo: v => {
    const s = (v||'').trim();
    if (!s)           return 'Vehicle number is required.';
    if (s.length < 4) return 'Enter a valid vehicle number.';
    return null;
  },
  experience: v => {
    if (!(v||'').trim()) return 'Experience is required.';
    return null;
  },
  address: v => {
    const s = (v||'').trim();
...
const DRIVER_FIELDS = [
  ...
  { key:'license',    label:'License No.', icon:'id-card-outline',  keyboard:'default', autoCapitalize:'characters', maxLength:16 },
  { key:'vehicleNo',  label:'Vehicle No.', icon:'car-outline',      keyboard:'default'   },
  { key:'experience', label:'Experience',  icon:'sparkles-outline', keyboard:'default'   },
  { key:'capacity',   label:'Capacity',    icon:'people-outline',   keyboard:'numeric'   },
];
...
const Field = ({label, value, onChange, keyboard, icon, error, autoCapitalize, maxLength}) => (
  <View style={s.fWrap}>
    ...
    <TextInput
      ...
      autoCapitalize={autoCapitalize || (keyboard==='phone-pad'||keyboard==='numeric'?'none':'sentences')}
      maxLength={maxLength}
    />
    {!!error && (
      <View style={s.errRow}>
...
export default function DriverProfileScreen({ refreshing, onRefresh }) {
  ...
  // ── Location states (same as registration) ────────────────────────────────
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!data?._id && !data?.email) throw new Error('Profile not found.');
      setProfile(data);
    } catch(e) {
  ...
  const startEdit = () => {
    setEditData({
      ...
      vehicleNo:   profile.vehicleNo   || profile.vehicle || '',
      vehicleType: profile.vehicleType || '',
      capacity:    profile.capacity    ? String(profile.capacity) : '',
      experience:  profile.experience  || '',
    });
    // pre-fill location fields from profile
  ...
  const runValidation = () => {
    const errs = {};
    ['name','phone','license','vehicleNo','experience'].forEach(k => {
      if (RULES[k]) { const m = RULES[k](editData[k]); if (m) errs[k] = m; }
    });
    // address / location
    const addrErr = RULES.address(address);
    if (addrErr) errs.address = addrErr;
    const cityErr = RULES.city(city);
    if (cityErr) errs.city = cityErr;
    const countryErr = RULES.country(country);
    ...
    <>
      <ScrollView style={{flex:1,backgroundColor:G.bg}} contentContainerStyle={{paddingBottom:40}}
        ...
        <View style={s.statsRow}>
          {[
            {val:profile.vehicleNo||profile.vehicle, color:G.main,    label:'Vehicle No'},
            ...
            {val:profile.capacity!=null ? String(profile.capacity) : '—', color:'#4CAF50', label:'Capacity'},
          ].map((item,i,arr) => (
        ...
        {!isEditing && (<>
          ...
          <Card icon="car-sport-outline" title="Vehicle Information">
            <Row icon="car-outline"     label="Vehicle No."  value={profile.vehicleNo||profile.vehicle}/>
            <Row icon="id-card-outline" label="License No."  value={profile.license}/>
            <Row icon="bus-outline"     label="Vehicle Type" value={profile.vehicleType ? cap(profile.vehicleType) : '—'}/>
            <Row icon="people-outline"  label="Capacity"     value={profile.capacity!=null ? String(profile.capacity) : '—'} last/>
          </Card>
          <Card icon="time-outline" title="Work Details">
            <Row icon="sparkles-outline" label="Experience" value={profile.experience} last/>
          </Card>
          {profile.availableTimeSlots?.length > 0 && (
            <Card icon="alarm-outline" title="Available Time Slots">
              <View style={{padding:14,flexDirection:'row',flexWrap:'wrap',gap:8}}>
                {profile.availableTimeSlots.map((slot,i) => (
                  <View key={i} style={s.slotChip}>
                    <Ionicons name="time-outline" size={12} color={G.main}/>
            ...
            <View style={{padding:14}}>
              ...
              {DRIVER_FIELDS.map(f => (
                <Field key={f.key} label={f.label} value={editData[f.key]}
                  onChange={v => { sf(f.key,v); if(f.key==='vehicleNo') sf('vehicle',v); }}
                  keyboard={f.keyboard} icon={f.icon} error={errors[f.key]}
                  autoCapitalize={f.autoCapitalize} maxLength={f.maxLength}/>
              ))}

              {/* Vehicle Type */}
              <View style={s.fWrap}>
                <View style={s.fLabelRow}><Ionicons name="bus-outline" size={11} color={G.main}/><Text style={s.fLabel}>Vehicle Type</Text></View>
                <View style={s.vpRow}>
                  {['car','van','bus'].map(v => {
                    const sel = editData.vehicleType === v;
                    return (
                      <TouchableOpacity key={v} style={[s.vpChip, sel&&s.vpChipSel]}
                        onPress={()=>sf('vehicleType',sel?'':v)} activeOpacity={0.8}>
              ...
              {/* ── Location Picker (same as registration) ── */}
              <View style={s.fWrap}>
                <View style={s.fLabelRow}>
              ...
              {/* City / Country / Zone — auto-filled, editable */}
...
const s = StyleSheet.create({
  ...
  slotChip:{flexDirection:'row',alignItems:'center',gap:5,paddingHorizontal:10,paddingVertical:6,borderRadius:9,backgroundColor:G.light,borderWidth:1,borderColor:G.border},
  slotTxt:{fontSize:12,fontWeight:'600',color:G.main},
